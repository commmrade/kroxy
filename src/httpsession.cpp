...
void HttpSession::run(Upstream &upstream)
{
    upstream_ = upstream;
...
void HttpSession::do_read_client_header(const boost::system::error_code &errc, [[maybe_unused]] std::size_t bytes_tf)
{
    if (!errc) {
        ...
        // --- choose upstream host (mock / simple pick the first) ---
        auto host = upstream_.hosts[0];

        // determine whether to use TLS for service (best-effort: port 443 => TLS)
        bool host_is_tls = upstream_.options.pass_tls_enabled.value_or(false);

        // create resolver bound to same executor as client socket
        ...
        // prepare SSL context for service stream (we move it into Stream)
        boost::asio::ssl::context service_ssl_ctx(boost::asio::ssl::context::sslv23_client);
        // Accept insecure by default to avoid verification failures in a mock environment.
        // In production, switch to verify_peer and configure CA paths.
        service_ssl_ctx.set_default_verify_paths();
        service_ssl_ctx.set_verify_mode(boost::asio::ssl::verify_none);
        if (upstream_.options.pass_tls_verify.value_or(false)) {
            service_ssl_ctx.set_verify_mode(boost::asio::ssl::verify_peer);
        }
        if ((upstream_.options.pass_tls_cert_path.has_value() && upstream_.options.pass_tls_key_path.has_value())) {
            service_ssl_ctx.use_certificate_chain_file(upstream_.options.pass_tls_cert_path.value());
            service_ssl_ctx.use_private_key_file(
              upstream_.options.pass_tls_key_path.value(), boost::asio::ssl::context::file_format::pem);
        }
        ...
        resolver->async_resolve(host.host,
          ...
          [self = shared_from_this(), resolver, host](
            const boost::system::error_code &errc, const boost::asio::ip::tcp::resolver::results_type &eps) {
              if (errc) {
              ...
              boost::asio::async_connect(self->service_sock_->socket(),
                ...
                [self, host](const boost::system::error_code &errc,
                  [[maybe_unused]] const boost::asio::ip::tcp::endpoint &endpoint) {
                    if (errc) {
                        std::println("Connecting to service failed: {}", errc.message());
                        self->close_ses();
                        return;
                    }
                    ...
                    if (self->service_sock_->is_tls()) {
                        if (!self->service_sock_->set_sni(host.host)) {
                            std::println("Warning: set_sni failed for host {}", host.host);
                            // not fatal necessarily; continue to handshake
                        }

                        self->service_sock_->async_handshake(
                          boost::asio::ssl::stream_base::client, [self](const boost::system::error_code &errc) {
                              if (errc) {
                                  std::println("Service TLS handshake failed: {}", errc.message());
                                  self->close_ses();
                                  return;
                              }
        ...
        // NOTE: don't attempt to write to service yet — wait until connection (and handshake) complete.
    } else {
...
void HttpSession::do_write_service_header(const boost::system::error_code &errc, [[maybe_unused]] std::size_t bytes_tf)
{
    if (!errc) {
...
void HttpSession::do_read_client_body(const boost::system::error_code &errc, [[maybe_unused]] std::size_t bytes_tf)
{
    if (boost::beast::http::error::need_buffer == errc || !errc) {
        ...
        service_sock_.async_write_message(
          *request_s_, [self = shared_from_this(), this](const boost::system::error_code &errc, std::size_t bytes_tf) {
        ...
        client_sock_.async_read_header(upstream_buf_,
          *request_p_,
          [self = shared_from_this(), this](const boost::system::error_code &errc,
            [[maybe_unused]] std::size_t bytes_tf) { do_read_client_header(errc, bytes_tf); });
        break;
    }
    case State::BODY: {
        client_sock_.async_read_message(upstream_buf_,
          *request_p_,
          [self = shared_from_this(), this](
            const boost::system::error_code &errc, std::size_t bytes_tf) { do_read_client_body(errc, bytes_tf); });
        break;
    }
    default: {
...
void HttpSession::do_read_service_header(const boost::system::error_code &errc, [[maybe_unused]] std::size_t bytes_tf)
{
    if (!errc) {
        ...
        client_sock_.async_write_header(*response_s_,
          [self = shared_from_this(), this](const boost::system::error_code &errc,
            [[maybe_unused]] std::size_t bytes_tf) { do_write_client_header(errc, bytes_tf); });
    } else {
        if (boost::beast::http::error::end_of_stream == errc || boost::asio::ssl::error::stream_truncated == errc) {
...
void HttpSession::do_read_service_body(const boost::system::error_code &errc, [[maybe_unused]] std::size_t bytes_tf)
{
    if (boost::beast::http::error::need_buffer == errc || !errc) {
        ...
        client_sock_.async_write_message(
          *response_s_, [self = shared_from_this(), this](const boost::system::error_code &errc, std::size_t bytes_tf) {
...
void HttpSession::do_downstream()
{
    ...
    case State::HEADERS: {
        ...
        service_sock_.async_read_header(downstream_buf_,
          *response_p_,
          [self = shared_from_this(), this](const boost::system::error_code &errc,
            [[maybe_unused]] std::size_t bytes_tf) { do_read_service_header(errc, bytes_tf); });
        break;
    }
    case State::BODY: {
        service_sock_.async_read_message(downstream_buf_,
          *response_p_,
          [self = shared_from_this(), this](const boost::system::error_code &errc,
            [[maybe_unused]] std::size_t bytes_tf) { do_read_service_body(errc, bytes_tf); });
        break;
    }
    default: {
        throw std::runtime_error("Not implemented");
