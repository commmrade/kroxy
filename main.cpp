...
class TlsHttpSession : public Session, public std::enable_shared_from_this<TlsHttpSession> {
private:
    template<bool isRequest, class Body>
    void process_headers(boost::beast::http::message<isRequest, Body> &msg) {
        // TODO: normal implementation, for now its mock
...
public:
    TlsHttpSession(boost::asio::io_context &ctx, boost::asio::ssl::context& ssl_ctx, HttpConfig &cfg) : cfg_(cfg), client_sock_(ctx, ssl_ctx), service_sock_(ctx) {
    }

    TlsHttpSession(const TlsHttpSession &) = delete;

    TlsHttpSession(TlsHttpSession &&) = delete;

    TlsHttpSession &operator=(const TlsHttpSession &) = delete;

    TlsHttpSession &operator=(TlsHttpSession &&) = delete;

    ~TlsHttpSession() override {
        close_ses();
    }

    void run() override {
        client_sock_.async_handshake(boost::asio::ssl::stream_base::server, [self = shared_from_this(), this] (const boost::system::error_code& errc) {
            if (!errc) {
                do_upstream();
                do_downstream();
            } else {
                std::println("Client TLS handshake failed: {}", errc.message());
            }
        });
    }
...
private:
    ...
    std::shared_ptr<Session> make_session() {
        if (cfg_.is_stream()) {
            ...
            const bool tls_enabled = true;
            if (tls_enabled) {
                return std::make_shared<TlsHttpSession>(ctx_, ssl_ctx_, std::get<HttpConfig>(cfg_.server_config));
            } else {
